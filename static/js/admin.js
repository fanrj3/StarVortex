...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (basicSettingsTab && advancedSettingsTab) {
        ...
        // 高级设置标签页点击事件
        advancedSettingsTab.addEventListener('click', () => {
    ...
    if (customFileTypesToggle && customFileTypesContainer) {
        customFileTypesToggle.addEventListener('change', function() {
            customFileTypesContainer.classList.toggle('hidden', !this.checked);
        });
    }
...
function renderAssignmentsList() {
    ...
    filteredAssignments.forEach(assignment => {
        ...
        // 获取高级设置信息
        const hasAdvancedSettings = assignment.advancedSettings ? true : false;
        const maxFileCount = hasAdvancedSettings ? assignment.advancedSettings.maxFileCount : 10;
        const maxFileSize = hasAdvancedSettings ? 
            `${assignment.advancedSettings.maxFileSize} ${assignment.advancedSettings.fileSizeUnit || 'MB'}` : 
            '256 MB';
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm">
                <div class="text-xs text-gray-500">
                    <span class="mr-2">
                        <i class="fas fa-file mr-1"></i>${maxFileCount}个文件
                    </span>
                    <span>
                        <i class="fas fa-hdd mr-1"></i>${maxFileSize}
                    </span>
                </div>
            </td>
...
function renderSubmissionsList(submissions) {
    ...
    // 更新统计信息
    /*if (stats) {
        submissionStats.textContent = `总共 ${stats.totalStudents} 名学生，已提交 ${stats.submittedCount} 人，提交率 ${stats.submissionRate}，截止日期: ${stats.dueDateStr}`;
        downloadAllBtn.disabled = submissions.length === 0;
    }*/
    ...
    submissions.forEach(submission => {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${submission.status === '逾期提交' ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'}">
                    ${submission.status}
                </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button 
                    ...
                    data-course="${submissionCourseFilter.value}"
                    data-assignment="${submissionAssignmentFilter.value}"
                >
                    查看详情
                </button>
                <a 
...
function collectAdvancedSettings() {
    ...
    const enableGrading = document.getElementById('enableGrading').checked;
    ...
    let allowedTypes = Array.from(allowedTypesCheckboxes).map(cb => cb.value).join(',').split(',');
    
    // 添加自定义文件类型
    if (document.getElementById('customFileTypesToggle').checked) {
        const customTypes = document.getElementById('customFileTypes').value;
        if (customTypes) {
...
function loadAdvancedSettings(settings) {
    ...
    // 设置评分选项
    if (settings.enableGrading !== undefined) {
        document.getElementById('enableGrading').checked = settings.enableGrading;
    }
    if (settings.enableFeedback !== undefined) {
...
function exportSubmissionStats() {
    const course = document.getElementById('submissionCourseFilter').value;
    const assignment = document.getElementById('submissionAssignmentFilter').value;
    ...
    fetch(exportUrl)
        ...
        .then(blob => {
            ...
            a.href = url;
            a.download = `${course}_${assignment}_提交统计_${new Date().toISOString().slice(0,10)}.xlsx`;
